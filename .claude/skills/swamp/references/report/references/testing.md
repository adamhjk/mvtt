...
Deno.test("report reads model data", async () => {
  ...
  const data = await context.dataRepository.findByName("aws/ec2", "my-ec2", "main");
...
Deno.test("report reads execution data via getContent", async () => {
  ...
  const raw = await context.dataRepository.getContent("aws/ec2", "my-ec2", "state-current", 1);
...
```typescript
import { createReportTestContext, withMockedFetch } from "@swamp-club/swamp-testing";
...
```typescript
import { createReportTestContext, withMockedCommand } from "@swamp-club/swamp-testing";
...
```typescript
import { createReportTestContext, withMockedFetch } from "../../packages/testing/mod.ts";
```
