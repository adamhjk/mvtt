  ...
  createProvider: (name: string, config: Record<string, unknown>) => {
    ...
    return {
      get: async (secretKey: string): Promise<string> => {
        ...
        if (!response.ok) {
          throw new Error(`Failed to get secret '${secretKey}': HTTP ${response.status}`);
        }
      ...
      put: async (secretKey: string, secretValue: string): Promise<void> => {
        ...
        if (!response.ok) {
          throw new Error(`Failed to put secret '${secretKey}': HTTP ${response.status}`);
        }
      },
      list: async (): Promise<string[]> => {
        const response = await fetch(`${baseUrl}/secrets`, { headers });
        if (!response.ok) {
          throw new Error(`Failed to list secrets: HTTP ${response.status}`);
        }
        const data = await response.json();
        return data.keys;
      },
...
export const vault = {
  ...
  createProvider: (name: string, config: Record<string, unknown>) => {
    ...
    const encrypt = async (plaintext: string): Promise<string> => {
      const keyBytes = Uint8Array.from(atob(parsed.encryptionKey), (c) => c.charCodeAt(0));
