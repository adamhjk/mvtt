...
export const datastore = {
  ...
  description: "Stores data in a custom local directory with file-based locking",
            ...
            while (Date.now() - start < maxWaitMs) {
              try {
                ...
                heartbeatId = setInterval(async () => {
                  try {
                    const current = JSON.parse(await Deno.readTextFile(lockFile));
                ...
                try {
                  const existing = JSON.parse(await Deno.readTextFile(lockFile));
