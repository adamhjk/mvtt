...
Same as halfling but unconditional re-roll-once on any value the player chooses — you can't express "player chooses" in notation, so reroll-once must be triggered by what the _system_ does. For "reroll any natural 1 once":
...
PF2e adds the _degree of success_ — crit success / success / failure / crit failure based on margin from DC. The notation just produces the d20+mod; the four-tier comparison is application logic.
...
The DSL's `r` / `ro` _replace_ the value (no choice). To take the better of original-and-reroll, you must split:
