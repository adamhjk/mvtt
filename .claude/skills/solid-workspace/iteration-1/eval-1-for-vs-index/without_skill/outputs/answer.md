        ...
        {(value, i) => (
          <div>
            <input
              value={value()}
              onInput={(e) =>
                setState("fields", i, e.currentTarget.value)
              }
            />
            <button
              onClick={() =>
...
4. Need both reorder *and* in-place edit? -> use `<For>` with stable object identities (don't `.map()` to new objects on every keystroke), or model the list with `createStore` so individual slots update without a parent-array replacement.
