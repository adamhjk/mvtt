...
import { CreateBlankNpc, CreateNpcFromCatalog, TbNpc } from "./shared/index.js";
import {
  Formula,
  RequestRoll,
  RolledBy,
...
describe("NpcsPageProvider — catalog picker (fuzzy search)", () => {
  it("renders the catalog rack with every TB NPC template visible by default", () => {
    const h = harness();
    mountWithClient(h, () => NpcsPageProvider.render({ tabId: TAB_ID, entityId: null }) as never);
    const opts = screen.getByTestId("npc-template-options");
    expect(
      opts.querySelector('[data-testid="npc-template-option-tb/npc/alchemist"]'),
    ).not.toBeNull();
    expect(opts.querySelector('[data-testid="npc-template-option-tb/npc/bandit"]')).not.toBeNull();
    expect(
      opts.querySelector('[data-testid="npc-template-option-tb/npc/beronin-bandit-chief"]'),
    ).not.toBeNull();
  });
  ...
  it("typing into the search filters the rack via subsequence fuzzy match", () => {
    const h = harness();
    mountWithClient(h, () => NpcsPageProvider.render({ tabId: TAB_ID, entityId: null }) as never);
    const search = screen.getByTestId("npc-template-search") as HTMLInputElement;
    fireEvent.input(search, { target: { value: "bndt" } });
    const opts = screen.getByTestId("npc-template-options");
    // "bndt" is a subsequence of "Bandit" (and "Bandit Chief, Dwarf"
    // role text); not of "Alchemist".
    expect(opts.querySelector('[data-testid="npc-template-option-tb/npc/bandit"]')).not.toBeNull();
    expect(opts.querySelector('[data-testid="npc-template-option-tb/npc/alchemist"]')).toBeNull();
  });
  ...
  it("clicking Spawn dispatches CreateNpcFromCatalog with the selected template id", () => {
    const h = harness();
    mountWithClient(h, () => NpcsPageProvider.render({ tabId: TAB_ID, entityId: null }) as never);
    fireEvent.click(screen.getByTestId("npc-template-option-tb/npc/alchemist"));
  ...
  it("each rendered row carries a clickable BookCitation chip for the printed page", () => {
    ...
    const banditRow = screen.getByTestId("npc-template-option-tb/npc/bandit");
    const citation = banditRow.querySelector('[data-canonical-id="tb/book/scholars-guide"]');
  ...
  it("an empty filter falls back to the inline empty-state, leaving the spawn button disabled", () => {
    ...
    const spawnBtn = screen.getByTestId("npc-spawn-submit") as HTMLButtonElement;
  ...
  it("the homebrew affordance dispatches CreateBlankNpc with the typed name", () => {
    const h = harness();
    mountWithClient(h, () => NpcsPageProvider.render({ tabId: TAB_ID, entityId: null }) as never);
    const blankInput = screen.getByTestId("npc-blank-name-input") as HTMLInputElement;
    fireEvent.input(blankInput, { target: { value: "Old Bran" } });
    fireEvent.click(screen.getByTestId("npc-blank-submit"));
    const dispatched = h.dispatched.find((d) => d.type === CreateBlankNpc.name);
    expect(dispatched).toBeTruthy();
...
describe("NpcsPageProvider — existing-NPCs hub list", () => {
  it("after spawning an NPC, the hub renders a row with the printed page citation", async () => {
    ...
    const citation = row!.querySelector('[data-canonical-id="tb/book/scholars-guide"]');
    expect(citation).not.toBeNull();
...
describe("Spawn-NPC palette commands", () => {
  ...
  it("each verb's hint cites the source book and printed page", () => {
    const alchemist = TB_SPAWN_NPC_PALETTE_COMMANDS.find((c) => c.label === "Spawn Alchemist");
  ...
  it("running a spawn verb dispatches CreateNpcFromCatalog and (after the spawn lands) OpenPageInNewTab onto the NPCs page with the new id", async () => {
    const h = harness();
    const verb = TB_SPAWN_NPC_PALETTE_COMMANDS.find((c) => c.label === "Spawn Alchemist")!;
    verb.run({ userId: "u", role: "gm", client: h.client });
    await new Promise((r) => setTimeout(r, 10));

    const spawnCmd = h.dispatched.find((d) => d.type === CreateNpcFromCatalog.name);
    expect(spawnCmd).toBeTruthy();
    expect(spawnCmd!.payload).toMatchObject({
      templateId: "tb/npc/alchemist",
    });

    const spawned = h.world.query([Character, TbNpc, Team]).find((row) => {
