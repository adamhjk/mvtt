...
/* -------------------------------------------------------------------------
 * Inventory — slot-based gear placement
 * ----------------------------------------------------------------------- */

const InventoryItem = z.object({
  name: z.string().max(120),
  /** Generic damaged / weariness markers shown on the printed sheet. */
  damaged: z.boolean().default(false),
  weariness: z.boolean().default(false),
});

const ContainerStatus = z.enum(["carried", "dropped", "lost"]);

/**
 * Slot-based inventory mirroring the printed sheet's layout. Each
 * named slot has a fixed capacity and a small per-slot status flag
 * where applicable (containers can be Carried / Dropped / Lost).
 *
 * Capacity numbers are encoded as array length on default-init; the
 * UI clamps writes to per-slot maxima (DH p.112). For shape-only the
 * substrate doesn't enforce capacity — that's a command validator job.
 */
export const Inventory = defineTrait({
  name: "@vtt/system-torchbearer/Inventory",
  schema: z
    .object({
      head: z.array(InventoryItem).max(1).default([]),
      neck: z.array(InventoryItem).max(1).default([]),
      handsWorn: z.array(InventoryItem).max(2).default([]),
      handsCarried: z.array(InventoryItem).max(2).default([]),
      torso: z.array(InventoryItem).max(3).default([]),
      belt: z.array(InventoryItem).max(3).default([]),
      feet: z.array(InventoryItem).max(1).default([]),
      pocket: z.string().max(240).default(""),
      satchel: z
        .object({
          isBackpack: z.boolean().default(false),
          items: z.array(InventoryItem).max(6).default([]),
          status: ContainerStatus.default("carried"),
        })
        .default({ isBackpack: false, items: [], status: "carried" }),
      largeSack: z
        .object({
          items: z.array(InventoryItem).max(6).default([]),
          status: ContainerStatus.default("carried"),
        })
        .default({ items: [], status: "carried" }),
      smallSackOne: z
        .object({
          items: z.array(InventoryItem).max(2).default([]),
          status: ContainerStatus.default("carried"),
        })
        .default({ items: [], status: "carried" }),
      smallSackTwo: z
        ...
        .default({ items: [], status: "carried" }),
      cache: z.array(InventoryItem).max(12).default([]),
    })
    .default({
      head: [],
      neck: [],
      ...
      feet: [],
      pocket: "",
      satchel: { isBackpack: false, items: [], status: "carried" as const },
      largeSack: { items: [], status: "carried" as const },
      smallSackOne: { items: [], status: "carried" as const },
      smallSackTwo: { items: [], status: "carried" as const },
      cache: [],
    }),
});
