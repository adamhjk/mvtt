...
describe("CharactersPageProvider", () => {
  ...
  it("lists existing characters in the hub", () => {
    const h = harness();
    h.world.spawn([Character({ name: "Aelric" }), Permissions(ownedBy(ME))]);
    h.world.spawn([Character({ name: "Tarn" }), Permissions(ownedBy(ME))]);
    mountWithClient(
      h,
      () =>
        CharactersPageProvider.render({
  ...
  it("renders the character sheet when entityId is set", () => {
    const h = harness();
    const id = h.world.spawn([Character({ name: "Tarn" }), Permissions(ownedBy(ME))]);
    mountWithClient(
      h,
      () =>
        CharactersPageProvider.render({
